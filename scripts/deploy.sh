...
hydrate_from_azure() {
  ...
  local current
  for name in AZURE_OPENAI_ENDPOINT AZURE_OPENAI_API_KEY AZURE_SEARCH_ENDPOINT AZURE_SEARCH_API_KEY AZURE_STORAGE_CONNECTION_STRING AZURE_STORAGE_CONTAINER PROJECT_HISTORY_BLOB_NAME DEFAULT_USER_ID REDIS_URL API_KEY SECRET_KEY GITHUB_TOKEN GITHUB_ORG; do
...
require_var AZURE_STORAGE_CONNECTION_STRING
...
COMMON_ENV_VARS=(
  ...
  "AZURE_STORAGE_CONTAINER=${AZURE_STORAGE_CONTAINER:-swarm-factory-state}"
  "PROJECT_HISTORY_BLOB_NAME=${PROJECT_HISTORY_BLOB_NAME:-project-history/state.json}"
  "DEFAULT_USER_ID=${DEFAULT_USER_ID:-default-user}"
  "REDIS_URL=${REDIS_URL}"
